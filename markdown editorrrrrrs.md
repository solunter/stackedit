...
---
在pad上目前没有发现什么好用的app. Joplin和Boostnote都不好使. MWeb倒是可以通过GoogleDrive导入笔记, 也支持LaTeX公式, 但是GoogleDrive同步又有bug. 现在就直接读`.md`原文件, 或者hexo部署到github查看, 但是就是改起来很麻烦了, 或者浏览器打开stackedit也行，渲染也可以看到， 还可以修改。手机就还是算了吧。但是pad上打开老是显示`you are offline`，不是很稳定，需要刷新一遍网页才行。
